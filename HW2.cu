...
// The maximum number of threads that can be held is 1024!
// Therefore if we need to go over that we will need threads to do more than one thing!
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	if(N < 1024)
	{
		...
		while(id + blockDim.x * k < N)
		{
			c[id + blockDim.x * k] = a[id + blockDim.x * k] + b[id + blockDim.x * k];
			k = k+1;
		}
			
			/*
			In this case I have made it so that if we go over the max number of threads,
			thread 0 will then try to do index 0 and then index 1024, then index 2048. Obviously if there
			is no index 2048 it will end! 
			*/
	}
}
