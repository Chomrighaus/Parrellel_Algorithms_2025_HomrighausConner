...
void setUpDevices()
{
	...
	int numberOfBlocks = int(((N - 1) / BlockSize.x)+1);
	GridSize.x = numberOfBlocks;
	GridSize.y = 1;
	GridSize.z = 1;
}
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	{
		c[id] = a[id] + b[id];
		printf("BlockIdx: %d, ThreadIdx: %d, A: %f, B: %f, C: %f \n", blockIdx.x, threadIdx.x, a[id], b[id], c[id]);
	}
	/*
	while(id < n)
	{
