...
__global__ void getForces(float3 *p, float3 *v, float3 *f, float *m, float g, float h, int n)
{
	...
	if (i < n)
	{
		...
		for (int tile = 0; tile < gridDim.x; tile++)
		{
			...
			{
				tile_p[threadIdx.x] = make_float3(0.0f, 0.0f, 0.0f);
			...
			for (int k = 0; k < blockDim.x; k++)
			{
				...
				f[i].x += force_mag * dx * inv_d;
				f[i].y += force_mag * dy * inv_d;
				f[i].z += force_mag * dz * inv_d;
			}
...
__global__ void moveBodies(float3 *p, float3 *v, float3 *f, float *m, float damp, float dt, float t, int n)
{
	...
	if (i < n)
	{
		...
		p[i].x += tile_v[threadIdx.x].x * dt;
		p[i].y += tile_v[threadIdx.x].y * dt;
		p[i].z += tile_v[threadIdx.x].z * dt;
